...
export const signUp = async (req, res) => {
  try {
    ...
    // Send the token inside the JSON!
...
export const login = async (req, res) => {
  try {
    ...
    if (!userExist) return res.status(400).json({ message: "Email does not exist!" });
...
export const logOut = async (req, res) => {
  try {
    // We don't need to clear cookies anymore! The frontend will handle deletion.
